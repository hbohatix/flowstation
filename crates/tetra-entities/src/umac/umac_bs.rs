            ...
            if let Some(ts) = traffic_ts {
                ...
                if !self.channel_scheduler.circuit_is_active(Direction::Dl, ts) {
                    ...
                    if hdr_len + sdu_len <= STCH_CAP {
                        ...
                        // Too large for one stolen half-slot: fragment across consecutive STCH
                        // half-slots (MAC-RESOURCE frag-start → MAC-FRAG… → MAC-END), exactly as
                        // the normal MCCH/SACCH path does. Before this, the whole SDU was copied
                        // into a fixed 124-bit buffer and `write_bits` PANICKED on overflow
                        // ("write would exceed buffer end"), crashing the entire stack — a
                        // remotely-triggerable DoS whenever an SDS/status longer than one
                        // half-slot was sent to an MS engaged in a call. (FH-BUG-034 follow-up.)
                        tracing::info!(
                            "rx_ul_tma_unitdata_req: FACCH stealing on ts {} — {} SDU + {} hdr bits exceeds {} STCH cap, fragmenting across stolen half-slots",
                            ts, sdu_len, hdr_len, STCH_CAP
                        );
                        ...
                        loop {
                            ...
                            // Safety backstop: a 124-bit slot always makes progress, so this
                            // bounds a hypothetical runaway rather than a real case.
                            if produced >= 32 {
                                tracing::error!(
                                    "FACCH stealing fragmentation exceeded {} blocks on ts {} — aborting",
                                    produced, ts
                                );
                                break;
                            }

...
/// engaged in a call (camped on a traffic timeslot's FACCH, not the MCCH). It is flushed onto the
/// MCCH — the normal, reliable idle-MS path — as soon as the destination leaves the call.
///
/// Rationale: FACCH-stealing an SDS to an in-call MS does not work in practice. A real D-SDS-DATA
/// exceeds one 124-bit stolen half-slot, so it must be MAC-fragmented across consecutive stolen
/// half-slots, and MS firmware does not reassemble fragments delivered over STCH — the message is
/// never received and the sender's SDS-TL delivery report (TL-ACK) times out ("message failed").
/// Deferring until the MS returns to the MCCH delivers it exactly like a normal idle-MS SDS, which
/// works and is acknowledged. (FH-BUG-034.)
#[derive(Debug, Clone)]
...
/// Maximum time an SDS may sit deferred while its destination stays in (back-to-back) calls before
/// it is dropped. Calls are bounded by the cell call-timeout (~2 min), so a normal call always
/// releases well within this; this only bounds pathological perpetual-call cases.
const MAX_SDS_DEFER: std::time::Duration = std::time::Duration::from_secs(300);
...
pub struct SdsBsSubentity {
    ...
    /// Individual SDS deferred until their destination leaves a call (see PendingSds).
...
impl SdsBsSubentity {
    ...
    /// Deliver any deferred SDS whose destination has left its call (returned to the MCCH).
    /// Called every tick. Items whose destination is still in a call are kept; items that have
    /// waited longer than MAX_SDS_DEFER are dropped (logged) rather than delivered unreliably.
    fn flush_pending_sds(&mut self, queue: &mut MessageQueue) {
        ...
        for p in std::mem::take(&mut self.pending_sds) {
            if !self.issi_on_local_traffic(p.dest_ssi) {
                tracing::info!(
                    "SDS: destination {} left its call — delivering deferred SDS on the MCCH",
                    p.dest_ssi
                );
                // dest is no longer on traffic, so send_d_sds_data takes the MCCH path (no re-defer).
                self.send_d_sds_data(queue, p.source_issi, p.dest_ssi, SsiType::Issi, p.user_defined_data);
            } else if p.queued_at.elapsed() > MAX_SDS_DEFER {
                tracing::warn!(
                    "SDS: dropping SDS {} -> {} deferred >{}s (destination stayed in call)",
                    p.source_issi,
                    p.dest_ssi,
                    MAX_SDS_DEFER.as_secs()
                );
            } else {
                self.pending_sds.push(p); // still in a call — keep waiting
            }
        }
    }
    ...
    pub fn tick_start(&mut self, queue: &mut MessageQueue, dltime: TdmaTime) {
        // Flush SDS that were deferred while their destination was in a call.
    ...
    ///
    /// For an INDIVIDUAL destination currently engaged in a call, the message is DEFERRED (queued)
    /// and delivered on the MCCH once the call releases — see PendingSds / flush_pending_sds for
    /// why FACCH-stealing an in-call SDS does not work. All other cases (idle ISSI, group/GSSI)
    ...
    fn send_d_sds_data(
        ...
        if dest_ssi_type == SsiType::Issi && self.issi_on_local_traffic(dest_ssi) {
            tracing::info!(
                "SDS: dest {} is in a call — deferring delivery until it returns to the MCCH",
                dest_ssi
            );
